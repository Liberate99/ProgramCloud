...
public class RedisUtil {
    ...
    private static String filePath = "src/main/resources/redis.properties";
    ...
    private static JedisPool getPool() {
        if (pool == null) {
            ...
            config.setMaxWaitMillis(Integer.valueOf(prop.getProperty("redis.MaxWaitMillis").trim()));
            //在borrow一个jedis实例时，是否提前进行validate操作；如果为true，则得到的jedis实例均是可用的；
            config.setTestOnBorrow(true);
            String redisIp = prop.getProperty("redis.host").trim();
            Integer redisPort = Integer.valueOf(prop.getProperty("redis.port").trim());
            String redisPass = prop.getProperty("redis.pass").trim();
            int timeOut = Integer.valueOf(prop.getProperty("redis.poolTimeout").trim());
            int database = Integer.valueOf(prop.getProperty("redis.database").trim());
            pool = new JedisPool(config, redisIp, redisPort, timeOut, redisPass, database);
